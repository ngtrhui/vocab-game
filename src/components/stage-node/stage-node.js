...
export const StageNode = forwardRef(
    ({ stage, status, align, onClick }, ref) => {
        ...
        const base = "w-14 h-14 rounded-full flex items-center justify-center transition-all duration-300 cursor-pointer active:scale-95";

        const styles = {
            done: `bg-gradient-to-br from-[#3A2A4D] to-[#2A1E3A] shadow-[0_6px_18px_rgba(168,85,247,0.35)]`,
            current: `bg-gradient-to-br from-violet-400 to-fuchsia-500 shadow-[0_0_35px_rgba(192,132,252,0.85)]`,
            locked: `bg-[#2A2236] opacity-40 grayscale cursor-not-allowed`,
        };
        ...
        return (
            ...
            >
                {status === "current" && (
                    <div
                        className={` absolute top-1/2 -translate-y-1/2 z-10 pointer-events-none drop-shadow-[0_0_25px_rgba(192,132,252,0.8)]
              ${offset}
              ${heroOffsetMap[align]}
              `}
                    >
